			...
			<div class="products-grid">
				...
				{:else}
					{#each filteredProducts as p (p.id)}
						...
						{@const inCartCount = getCartItemCount(p.id)}
						{@const isOutOfStock = (p.stockQty ?? 0) <= 0}
						{@const isLowStock = (p.stockQty ?? 0) > 0 && (p.stockQty ?? 0) <= (p.lowStockThreshold ?? 5)}
						...
						>
							...
							<div class="card-top-row">
								<div class="card-icon {p.categorySlug ?? 'other'}">
									<CatIcon size={16} />
								</div>
								<div class="card-stock">
									...
									{:else if isLowStock}
										<span class="badge status-pending">{p.stockQty} left</span>
									{:else}
										<span class="stock-pill">{p.stockQty} {p.unit ?? 'pcs'}</span>
									{/if}
								</div>
							</div>

							<div class="card-body-content">
								<h3 class="card-prod-title">{p.name}</h3>
								<span class="card-sku mono">{p.sku}</span>
							</div>

							<div class="card-price-row">
								<span class="card-price mono">{money(p.priceCents)}</span>
								<span class="add-icon-pill">

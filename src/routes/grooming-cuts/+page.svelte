	...
	{:else}
		<div class="cut-cards-grid">
			{#each filteredCutCards as card (card.id)}
				{@const behavior = getBehaviorLabel(card.behaviorScore)}
				<div class="card cut-card">
					...
					<div class="cut-card-top spread">
						<div class="pet-header-info">
							<PetAvatar
								name={card.petName}
								species={card.petSpecies}
								breed={card.petBreed}
								size="md"
							/>
						</div>

						<!-- Behavior Rating Star Badge -->
					...
					<div class="specs-body stack-sm">
						...
						<div class="spec-block purple-tint">
							<div class="spec-label">
								<Scissors size={13} class="icon-indigo" />
								<span>Body Blade / Length:</span>
							</div>
							<div class="spec-value cell-strong">
								{card.bladeLengthBody || "Standard Breed Clip"}
							</div>
						</div>
						...
						{#if card.bladeLengthFace}
							<div class="spec-block blue-tint">
								<div class="spec-label">
									<Smile size={13} class="icon-blue" />
									<span>Face &amp; Head Blueprint:</span>
								</div>
								<div class="spec-value">
									{card.bladeLengthFace}
								</div>
							</div>
						{/if}
						...
						{#if card.scissorNotes}
							<div class="spec-block neutral-tint">
								<div class="spec-label">
									<Tag size={13} class="muted" />
									<span>Scissor Finishing &amp; Shape:</span>
								</div>
								<div class="spec-value small">
									{card.scissorNotes}
								</div>
							</div>
						{/if}
							...
							<div class="health-notes-box">
								{#if card.coatCondition}
									<div class="health-line">
										<span class="health-label">Coat:</span>
										<span class="health-val">{card.coatCondition}</span>
									</div>
								{/if}
								{#if card.skinIssues}
									<div class="health-line">
					...
					<div class="cut-card-footer spread">
						...
						<div class="row gap-1">
							<button
								...
								type="button"
								title="Edit Styling Specs"
								onclick={() => openEditCutCardModal(card)}
							>
								<Pencil size={13} />
								<span>Edit</span>
							</button>
							<form method="POST" action="?/deleteCutCard" onsubmit={(e) => { if (!confirm(`Delete grooming cut card for ${card.petName}?`)) e.preventDefault(); }}>

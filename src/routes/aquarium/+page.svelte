			...
			{:else}
				<div class="tanks-grid">
					{#each filteredTanks as tank}
						{@const eco = getEcosystemBadge(tank.ecosystem)}
						<div class="card tank-card">
							...
							<div class="tank-specs-box">
								<div class="specs-grid">
									{#if tank.dimensions}
										<div class="spec-item">
											...
											<span class="spec-v">{tank.dimensions}</span>
										</div>
									{/if}
									{#if tank.filtrationType}
										<div class="spec-item">
											<span class="spec-k">Filtration</span>
											<span class="spec-v" title={tank.filtrationType}>{tank.filtrationType}</span>
										</div>
									{/if}
									{#if tank.lightingType}
										<div class="spec-item">
											<span class="spec-k">Lighting</span>
											<span class="spec-v" title={tank.lightingType}>{tank.lightingType}</span>
										</div>
									{/if}
								</div>
								...
								{#if tank.notes}
									<p class="tank-notes">
										<span class="notes-tag">Notes:</span> {tank.notes}
									</p>
								{/if}
							</div>

							<!-- Latest Water Parameters Telemetry -->
							<div class="telemetry-box">
								<div class="spread align-center telemetry-head">
									<span class="telemetry-title">
										<Activity size={13} class="primary" />
										Latest Chemistry Telemetry
									</span>
									{#if tank.latestLog}
										<span class="telemetry-time" title={formatDate(tank.latestLog.recordedAt, true)}>
											{formatDate(tank.latestLog.recordedAt)}
										</span>
									{/if}
								</div>
								...
								{#if tank.latestLog}
									...
									<div class="params-pill-grid">
										<div class="param-cell {phStatus.class}">
											<span class="p-label">pH</span>
											<span class="p-val">{phStatus.text}</span>
										</div>
										<div class="param-cell {ammStatus.class}">
											<span class="p-label">NH₃/NH₄</span>
											<span class="p-val">{ammStatus.text}</span>
										</div>
										<div class="param-cell {nitStatus.class}">
											<span class="p-label">NO₂</span>
											<span class="p-val">{nitStatus.text}</span>
										</div>
										<div class="param-cell {ntrStatus.class}">
											<span class="p-label">NO₃</span>
											<span class="p-val">{ntrStatus.text}</span>
										</div>
										{#if tank.ecosystem === "reef" || tank.ecosystem === "marine"}
											<div class="param-cell {salStatus.class}">
												<span class="p-label">Salinity</span>
												<span class="p-val">{salStatus.text}</span>
											</div>
										{/if}
										{#if log.temperatureC}
											<div class="param-cell {tempStatus.class}">
												<span class="p-label">Temp</span>
												<span class="p-val">{tempStatus.text}</span>
											</div>
										{/if}
										{#if log.khDkh}
											<div class="param-cell param-info">
												<span class="p-label">KH</span>
												<span class="p-val">{log.khDkh} dKH</span>
											</div>
										{/if}
									</div>
								{:else}
									<div class="no-telemetry-notice">
										<Info size={13} />
										<span>No water test recorded yet for this setup.</span>
									</div>
								{/if}
							</div>

							<!-- Service Plan Status & Card Actions -->
							<div class="tank-card-foot spread">
								...
								<div class="row gap-1">
									<button
										...
										onclick={() => openLogTest(tank.id)}
										title="Log water parameter chemistry"
									>
									...
									{#if !tank.activePlan}
										<button
											...
											onclick={() => openCreatePlan(tank.id, tank.ownerId)}
											title="Set up recurring service plan"
										>
											<Repeat size={13} />
											<span>+ Plan</span>
										</button>
